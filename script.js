...
class Transaction {
    ...
    isValid() {
        ...
        if (!this.signature || this.signature.length === 0) {
            throw new Error('Pas de signature pour cette transaction');
        }

        return true; // Simplification de la vérification
    }
}
...
class Blockchain {
    ...
    minePendingTransactions(miningRewardAddress) {
        // Créer 2 blocs maximum avec 2 transactions chacun
        for (let i = 0; i < 2; i++) {
            const blockTransactions = this.pendingTransactions.slice(i * 2, (i + 1) * 2);
            
            if (blockTransactions.length === 0) break;
        ...
        this.pendingTransactions = this.pendingTransactions.filter(tx => tx.fromAddress !== null);
    }

    addTransaction(transaction) {
        if (!transaction.fromAddress || !transaction.toAddress) {
            throw new Error('Transaction doit avoir une adresse source et destination');
        }
...
function demonstrateBlocchain() {
    ...
    const transactions = [
        new Transaction(walletA.getPublicKey(), walletB.getPublicKey(), 50),
    ...
    transactions.forEach(tx => {
        const senderWallet = [walletA, walletB, walletC, walletD, walletE]
            .find(w => w.getPublicKey() === tx.fromAddress);
    ...
    console.log('Solde de walletA:', maCrypto.getBalanceOfAddress(walletA.getPublicKey()));
    console.log('Solde de walletB:', maCrypto.getBalanceOfAddress(walletB.getPublicKey()));
    console.log('Solde de walletC:', maCrypto.getBalanceOfAddress(walletC.getPublicKey()));
    console.log('Solde de walletD:', maCrypto.getBalanceOfAddress(walletD.getPublicKey()));
    console.log('Solde de walletE:', maCrypto.getBalanceOfAddress(walletE.getPublicKey()));
